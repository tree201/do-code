...
export async function createInteractiveSessionStore(options: {
  ...
  runtime?: () => { session: SavedSession; modelConfig: RuntimeModelConfig } | undefined
  ...
  const runtimeState = () => options.runtime?.() ?? { session: activeSession, modelConfig: initialModelConfig }
  ...
  const performSave = async (force = false) => {
    ...
    const metadata = { id: activeSession.id, workspace: options.workspace, model: runtime.modelConfig.preset, ...(title ? { title } : {}), createdAt: activeSession.createdAt ?? updatedAt, updatedAt, messageCount: messages.length }
