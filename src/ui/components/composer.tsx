...
export function Composer({ running, input, activity, queue, suggestions, attachments, status, statusRight }: ComposerProps) {
  return (
    <Box flexDirection="column" flexGrow={0} flexShrink={0} marginTop={2}>
      ...
      {queue ? <Box flexDirection="column" paddingLeft={3} paddingRight={1}>{queue}</Box> : null}
