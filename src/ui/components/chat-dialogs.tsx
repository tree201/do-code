...
export function ChatDialogs({ props, state }: { props: ChatAppProps; state: ChatAppState }) {
  ...
  return <>
    ...
    {dialog.kind === "model" && state.runtimeStore.canSwitchModel ? <ModelDialog models={state.activeModelPresets} currentModel={state.activeModel} language={state.activeLanguage} onClose={() => state.setActiveDialog({ kind: "none" })} onSelect={state.runtimeStore.switchModel} {...(state.runtimeStore.canPersistDefaultModel ? { onPersist: state.runtimeStore.persistDefaultModel } : {})} /> : null}
