...
test("composer owner applies synchronous edits without state-ref mirrors", () => {
  ...
  owner.setNodes([{ kind: "image", reference: "attachments/a.png", name: "a.png", size: 10 }])
  owner.setQueuedInputs([{ value: "next", nodes: [] }])
  const snapshot = owner.getSnapshot()
  assert.equal(snapshot.editor.value, "draft!")
  assert.equal(snapshot.completionIndex, 0)
  assert.equal(snapshot.nodes[0]?.kind, "image")
  assert.deepEqual(snapshot.queuedInputs, [{ value: "next", nodes: [] }])
  owner.destroy()
})
