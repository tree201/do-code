...
test("checks stable and preview npm dist-tags", async () => {
  const fetcher: typeof fetch = async () => new Response(JSON.stringify({ "dist-tags": { latest: "9.0.0", next: "10.0.0-next.1" } }), { status: 200, headers: { "content-type": "application/json" } })
  const stable = await checkForUpdate("stable", fetcher)
  assert.equal(stable.current, DO_CODE_VERSION)
  assert.equal(stable.latest, "9.0.0")
  assert.equal(stable.updateAvailable, true)
  const preview = await checkForUpdate("preview", fetcher)
  assert.equal(preview.tag, "next")
  assert.equal(preview.latest, "10.0.0-next.1")
})
