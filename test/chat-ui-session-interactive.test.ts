...
test("language command switches the live interface with second-level completion", async () => {
  ...
  view.stdin.write("/language ")
  await tick();await tick()
  const languageSuggestions = stripVTControlCharacters(view.lastFrame() ?? "")
  assert.match(languageSuggestions, /ja.*将界面与输出语言切换为日本語/)
  assert.doesNotMatch(languageSuggestions, /Set interface and output language/)
  view.stdin.write("\u0015")
  view.stdin.write("/memory ")
  await tick();await tick()
  assert.match(stripVTControlCharacters(view.lastFrame() ?? ""), /list.*列出已加载的 AGENTS\.md 指令文件/)
  view.stdin.write("\u0015")
  view.stdin.write("/export ")
  await tick();await tick()
  assert.match(stripVTControlCharacters(view.lastFrame() ?? ""), /md.*将会话导出为 Markdown/)
  view.stdin.write("\u0015")
