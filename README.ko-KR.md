...
- **짧은 작업 노트로 재개** — 여러 단계에 걸쳐 진행 상황을 남겨야 하면 선택적 `TASK.md`에 목표, 진행, 증거, 차단 요소, 다음 단계를 기록할 수 있습니다.
...
`/stats`로 컨텍스트 사용량을 확인하고 `/compact`로 필요할 때 압축하세요. 한도에 가까워지면 do-code는 오래된 완전한 턴을 묶어서 요약하고 롤링 요약과 최근의 완전한 작업 큐를 유지합니다. 지속적인 인계가 필요한 작업은 루트의 짧은 `TASK.md`에 기록할 수 있으며, 매 모델 요청 전에 다시 읽습니다.
