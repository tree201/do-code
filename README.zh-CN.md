...
- **用简短工作笔记恢复任务**：需要跨步骤持续记录时，可使用 `TASK.md` 保存目标、进展、证据、阻塞和下一步。
...
使用 `/stats` 查看上下文用量，使用 `/compact` 手动压缩。接近上下文上限时，do-code 会批量归纳较早的完整回合，同时保留滚动摘要和最近完整任务队列。需要持久交接时，可在项目根目录维护简短的 `TASK.md`；每次模型请求前都会重新读取。
