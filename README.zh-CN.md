...
- **任务执行时继续工作**：排队消息现在固定显示在输入框正上方，可检查内容或按 `↑` 重新调出。
- **安全处理大段粘贴**：很长的粘贴文本会在输入框中折叠为紧凑预览，不再挤走输入区域。
- **可靠阅读终端 Markdown**：超长行内代码、URL、无空格中文和 emoji 会按终端显示宽度换行，不丢失内容。
- **让审批偏好随会话保留**：每个会话都会记住审批模式；工作时可使用左手快捷键 `Ctrl+G` 切换。
- **让长任务保持聚焦**：压缩时保留滚动摘要和最近完整任务回合，较早工作会批量归纳，当前操作仍保留原文。
- **用简短工作笔记恢复任务**：需要跨步骤持续记录时，Agent 会自动维护任务笔记，保存目标、进展、证据、阻塞和下一步，笔记存储在工作区之外。
...
使用 `/stats` 查看上下文用量，使用 `/compact` 手动压缩。接近上下文上限时，do-code 会批量归纳较早的完整回合，同时保留滚动摘要和最近完整任务队列。需要持久交接时，Agent 可维护简短的任务笔记；每次模型请求前都会重新读取。
