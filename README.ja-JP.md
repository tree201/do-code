...
- **短い作業メモから再開** - ステップをまたいで進捗を残す必要があるとき、エージェントが目標、進捗、証拠、ブロッカー、次の手順を作業ノートとして自動管理し、ワークスペース外に保存します。
...
`/stats` でコンテキスト使用量を確認し、必要に応じて `/compact` で圧縮します。上限が近づくと、do-code は古い完全なターンをまとめて要約し、ローリング要約と最近の完全なタスクキューを保持します。永続的な引き継ぎが必要な作業では、エージェントが短い作業ノートを維持でき、モデルへの各リクエスト前に再読み込みされます。
